...
execute if score @s tbs.particle_decay matches 1 run scoreboard players add @s tbs.entity_temp 1
scoreboard players add @s[tag=tbs.sign.empty] tbs.particle_temp 1
kill @s[tag=tbs.sign.empty,scores={tbs.particle_temp=1000..}]
execute unless entity @s[tag=tbs.sign.empty] if score @s tbs.entity_temp matches 0 run tag @s add tbs.sign.empty
