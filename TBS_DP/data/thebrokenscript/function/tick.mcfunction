...
execute as @a[scoes={tbs.fake_ban=1..}] run function thebrokenscript:zzz/3
scoreboard players remove @e[tag=tbs.particles] tbs.particle_decay 1
kill @e[tag=tbs.particles,scores={tbs.particle_decay=..0}]
execute as @e[tag=tbs.structure.all_dead_replacer] at @s run function thebrokenscript:zzz/4
execute if predicate {"condition":"minecraft:random_chance","chance":0.0001} at @r run function thebrokenscript:zzz/5
execute as @e[tag=tbs.sign] at @s run function thebrokenscript:sign_comms/as_sign
execute if score enabled tbs.moon_event matches 1 if predicate {"condition":"minecraft:random_chance","chance":0.0004} as @e[type=#thebrokenscript:despawnable,limit=30,nbt={PersistenceRequired:0b}] run function thebrokenscript:zzz/6
scoreboard objectives add tbs.replace_temp dummy
execute as @e[type=#thebrokenscript:despawnable,tag=!tbs.entity.replace_checked,limit=30,nbt={PersistenceRequired:0b}] at @s run function thebrokenscript:zzz/7
...
execute as @a at @s if block ~ ~ ~ #minecraft:beds if predicate thebrokenscript:sleep_condition run function thebrokenscript:zzz/9
execute at @a run fill ~-5 ~-5 ~-5 ~5 ~5 ~5 stone replace deepslate
execute as @e[type=villager,tag=!tbs.entity.testificate_check] at @s run function thebrokenscript:zzz/10
